...
from unicharm.export_custom_data._export_custom_data import ExportPlan, ExportReportByDate
...
class ExportByDateAPIView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        import datetime
        time = str(datetime.datetime.now()).replace("-","").replace(":","").replace(" ","_").split(".")[0]
        time_file_name = "report_"+time
        ...
        old_token = new_export_file_path.split('/')[-1].split('.')[-2]
        new_export_file_path = new_export_file_path.replace(old_token,time_file_name)
        # export_data.save()
        # return Response(str(time_))

        if data.get('export_type') == 'BY_CUSTOMER':
        ...
        elif data.get('export_type') == 'BY_DATE':
            data = ExportReportByDate.export_plan(
            data.get('start_date'),
            data.get('end_date'),
            data.get('province_code'),
            str(data.get('campaign_sid')).replace("-","") if data.get('campaign_sid') else '',
            ...
            new_export_file_full_path = get_local_file_full_path(
                new_export_file_path)
            ensure_dir(new_export_file_full_path)
            writed, errors = ExportReportByDate.write_data_to_file(
                data, new_export_file_full_path)
            if writed is True:
                export_data.status = ExportData.DONE
                export_data.export_file.name = new_export_file_path
